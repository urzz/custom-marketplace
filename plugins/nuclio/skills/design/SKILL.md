...
- Run Bootstrap Check first with `node "$CLAUDE_PLUGIN_ROOT/scripts/bootstrap-check.mjs" --requested-skill design` and confirm the target change is the active, valid workflow before design writes.
- `plan.yaml` tasks must include `type`, `risk`, `allowed_paths`, `forbidden_paths`, `acceptance`, `verify`, and `review_focus` when applicable, because Build and guard use those fields for scope control; validate with `node "$CLAUDE_PLUGIN_ROOT/scripts/validate-plan.mjs" .nuclio/changes/<change-id>/plan.yaml --section tasks`.
- Design Approval must update canonical change state with `phase: "build"`, `approved.design: true`, and `gate: null` only after explicit user approval; write it through `node "$CLAUDE_PLUGIN_ROOT/scripts/write-state.mjs" <state-file> '<json>'`.
- Append typed `design.generated` and `design.approved` events with `node "$CLAUDE_PLUGIN_ROOT/scripts/append-event.mjs" .nuclio/changes/<change-id>/events.jsonl '<json>'` when those facts occur.
