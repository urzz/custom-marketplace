...
- Project-level `.dev-docs/` 只保留 index、knowledge、archive 和 changes/index：`.dev-docs/index.md`、`.dev-docs/index.json`、`.dev-docs/knowledge/`、`.dev-docs/archive/**` 与 `.dev-docs/changes/index.md`。`.dev-docs/changes/index.md` 是受控 `index_targets`，不是 change-local evidence。
...
- Finish target language metadata 是长期 knowledge/index/archive 写入时的目标语言 authority：existing target 保持既有主要语言，new target 使用 Contract-bound `output_language`，unknown/missing metadata 必须 STOP。
