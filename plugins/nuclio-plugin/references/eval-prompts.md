...
- Completion handoff must be `CHANGE_ROOT/evidence/completion.md` and `CHANGE_ROOT/evidence/completion.json`; Finish decision handoff must be `CHANGE_ROOT/evidence/decision.md` and `CHANGE_ROOT/evidence/decision.json`; Finish plan must be `CHANGE_ROOT/evidence/finish-plan.json`; root-level same-name handoff files are not active authority and must not be fallback; Finish apply journal evidence must be `CHANGE_ROOT/evidence/finish-apply.json` with optional prose `CHANGE_ROOT/evidence/finish-apply.md`.
