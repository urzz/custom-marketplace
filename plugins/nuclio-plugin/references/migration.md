...
Migration 定义 Nuclio 对 legacy `.dev-docs/` 或其它旧工作流 artifact 的 detect、preview、apply 合同。默认路径只读 preview；apply 必须显式 opt-in、完整验证、原子写入新 artifacts、不覆盖 legacy、不改产品代码。任何 authority 字段缺失都拒绝 partial conversion。
...
Preview 至少必须覆盖以下 Nuclio 字段，且逐项说明 source 或 blocker。
