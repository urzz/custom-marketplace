...
HELPER = ROOT / "plugins" / "nuclio-plugin" / "scripts" / "migration-helper.py"
VALID_HASH = "a" * 64
OTHER_HASH = "b" * 64
THIRD_HASH = "c" * 64
...
def context_entry(entry_id="worker-impl", audience="worker", mode="jit", path="plugins/nuclio-plugin/scripts/context-helper.py", **overrides):
...
def write_complete_legacy(change):
    ...
    tasks:
      - id: T1
        ...
        dependencies: []
        mutation_targets:
          - path: plugins/nuclio-plugin/scripts/generated.py
            mode: create
    ...
    """).strip() + "\n", encoding="utf-8")
    write_jsonl(change / "context" / "implement.jsonl", [context_entry(), context_entry("contract-ref", "contract", "stable", "plugins/nuclio-plugin/references/lifecycle.md", sha256=VALID_HASH)])
    write_jsonl(change / "context" / "verify.jsonl", [context_entry("review-ref", "reviewer", "stable", "plugins/nuclio-plugin/references/state-protocol.md", sha256=OTHER_HASH)])
...
class MigrationHelperTests(unittest.TestCase):
    ...
    def test_missing_ownership_validation_and_context_conflict_block_without_partial_authority(self):
        ...
        write_complete_legacy(self.legacy)
        write_jsonl(self.legacy / "context" / "verify.jsonl", [context_entry("worker-impl", "reviewer", "stable", "plugins/nuclio-plugin/references/state-protocol.md", sha256=OTHER_HASH)])
        preview = self.helper.preview_migration(self.legacy, self.target)
        self.assertFalse(preview["ready_to_apply"])
