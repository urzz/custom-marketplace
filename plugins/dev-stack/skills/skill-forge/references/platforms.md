...
| 项目 | Claude Code | Codex | 双平台 |
|---|---|---|---|
| 技能内容 | `skills/<name>/SKILL.md` | 同左 | 共用正文和 supporting files |
| 插件入口 | `.claude-plugin/plugin.json` | `.codex-plugin/plugin.json` | 两份清单、同一插件标识与版本 |
| Marketplace | `.claude-plugin/marketplace.json` | `.agents/plugins/marketplace.json` | 两个索引指向同一插件根目录 |
| 显式调用 | `/plugin:skill` | `$plugin:skill`；独立技能为 `$skill` | 分别记录实际入口 |
| 仅显式调用 | `disable-model-invocation: true` | `agents/openai.yaml` 的 `policy.allow_implicit_invocation: false` | 分别声明并保留既有行为 |
| 原生代理配置 | 插件根级 `agents/*.md` | 宿主已配置的 `.codex/agents/*.toml` 或原生子代理工具 | 共享角色要求，按宿主派发 |
...
本技能以 `/dev-stack:skill-forge`（Claude Code）或 `$dev-stack:skill-forge`（Codex）显式调用。正文中的 `/skill-forge` 是简称。

`SKILL_FORGE_DIR` 表示本次实际加载的 `SKILL.md` 所在绝对目录：Claude Code 取 `${CLAUDE_SKILL_DIR}`；Codex 取技能目录清单中的实际路径。它不是自动注入的环境变量。每次命令都使用核实并正确引用的绝对路径，或在同一次 shell 调用内设置任务变量，不依赖上一次 shell 的赋值。

`SKILL_FORGE_REPO_ROOT` 表示用户指定项目或宿主本次会话的目标仓库目录，不从插件路径或后来变化的 CWD 推断；它同样是每次调用需要落实的路径记号。`.skill-forge/<run>/` 始终位于目标仓库；validator 的 Spec/Plan 参数使用目标仓库内的绝对路径，`--repo-root` 显式传入目标仓库。插件缓存只读。

Claude Code 使用原生提问入口；Codex 展示问题并等待明确回复，或使用当前宿主明确允许用于该问题的交互工具。确认前停止依赖该答案的写入。空答案、默认选项、超时都不表示同意。不为使用仅限 Plan Mode 的提问工具而切换模式；在只读或 Plan mode 中只分析，不写 Spec、Plan 或目标文件。
...
Claude Code 目标运行原生 `claude plugin validate <plugin> --strict`。Codex 目标验证清单、`SKILL.md`、`openai.yaml`，并用实际客户端加载；CLI 的本地目录 RPC 可验证名称、来源路径和技能元数据，不代表模型行为评测。双平台目标两者都执行。
