...
description: "Use when applying one authorized bounded fix attempt for skill-forge OPEN blocking findings within declared ownership."
...
Do not mark findings RESOLVED. A fresh reviewer must verify closure.
...
- the complete same-Gate/same-owner OPEN blocking finding IDs;
- full finding details for every authorized ID;
- closure_test.expected for every authorized finding;
- JSON report path;
- expected fix commit message summary or enough finding summary text to form it;
...
do not modify product files or commit; this is a pre-mutation path.
...
review-state.json, rubric snapshots, ledgers, plans, specs, or unowned files.
...
1. Confirm the authorized ID set is complete and matches the provided finding
   details exactly.
2. Confirm all required_fix_paths are inside the same Task ownership.
3. Confirm all closure tests are concrete commands with expected exit code and
   output.
4. Confirm the current HEAD equals base_head_sha.
5. Confirm the JSON report path is available.
...
that would make the closure test pass.
...
Apply the minimal diff that closes all authorized findings together. Do not choose
a subset. Do not split the attempt across multiple commits. Do not introduce
future Task behavior or unrelated cleanup.

If any post-mutation non-success outcome becomes necessary (BLOCKED,
NO_PROGRESS, or a newly discovered NEEDS_CONTEXT), first restore only this attempt's
...
has been restored, write the non-success JSON report and do not commit. If you
cannot safely restore any touched product path, do not commit; write the report
with the unrecovered dirty paths, state that the Controller must HALT/manual
handling, and do not claim the checkout is clean.
...
A FIXED report requires all authorized closure tests to have run. If any closure
test fails and cannot be fixed within ownership, follow the post-mutation
non-success cleanup policy and report BLOCKED or NO_PROGRESS as appropriate
instead of committing.

## Commit Contract

Create exactly one fix commit only after all closure tests pass:
...
- still write the JSON report path as an uncommitted handoff for the Controller
  to read;
- use exactly `fix(<scope>): [Task <task_id>] <finding-summary>`;
- do not include RESOLVED, PASS, or APPROVED in the message;
- do not create a second commit.

The summary should identify the authorized finding set without inventing new
requirements.
...
Put human-readable reason codes in the final response or surrounding handoff,
not as extra JSON fields that the helper cannot import.
...
unrecovered_dirty_paths and `controller_action: "HALT_MANUAL_HANDLING"`, state
the same in the final response, and require the Controller to HALT/manual
handling before any helper import or further agent dispatch.
...
unrecovered dirty paths and require Controller HALT/manual handling before
further automation. Use FIXED only after all authorized closure tests passed and
the single fix commit exists.
